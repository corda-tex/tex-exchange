...
class OrderListFlowTests {
    ...
    fun `OrderListFlow will not find not existent stock`() {
        val flow = OrderListFlow(UniqueIdentifier(), ONE_POUND, 1, ONE_DAY)
    ...
    fun `OrderListFlow publishes and sends to counter party`() {
        ...
        val issuedStockId = sellerNode.startFlow(SelfIssue.SelfIssueStockFlow(null, "stock description", 1)).get()
        val stock = sellerNode.services.vaultService.queryBy(Stock::class.java).states[0].state.data
        assert(issuedStockId == stock.stockId)

        val orderListFlow = OrderListFlow(stock.stockId, ONE_POUND, 1, ONE_DAY)
